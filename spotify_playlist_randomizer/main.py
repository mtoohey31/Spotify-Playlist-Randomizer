...
class SpotifyPlaylistRandomizer(object):
    ...
    def randomize_playlist(self, source_playlist_id,
        ...
        logger.info(
            'Getting a fresh set of 100 tracks from the source playlist')
...
def main():
    parser = ap.ArgumentParser(prog="Spotify-Playlist-Randomizer")
    parser.add_argument("username", nargs=1)
    parser.add_argument("source_playlist_id", nargs=1)
    parser.add_argument("dest_playlist_id", nargs=1)
    args = parser.parse_args()

    logging.basicConfig(level=logging.INFO)

    randomizer = SpotifyPlaylistRandomizer(username=args.username)
    randomizer.randomize_playlist(
        source_playlist_id=args.source_playlist_id,
        destination_playlist_id=args.dest_playlist_id
    )
